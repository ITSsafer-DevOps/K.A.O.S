...
def print_banner():
    ...
    print(f"{Fore.RED}⚡ Starting Kali Linux AI environment...{Style.RESET_ALL}")
    time.sleep(0.5)
...
def ensure_persistence():
    ...
    try:
        ...
        with open(SESSION_LOG, "a") as f:
            f.write(f"[{timestamp}] Session started. Brain: {BRAIN_HOST}:{BRAIN_PORT}\n")
    except IOError as e:
        print(f"{Fore.RED}ERROR: Failed to write to persistent storage: {e}{Style.RESET_ALL}")
...
def brain_analysis(command_input):
    ...
    try:
        ...
        return {"proposed_command": proposed_cmd, "reasoning": "Offline fallback rule applied."}
...
def get_kali_prompt():
    ...
    line1 = (
        f"{Fore.BLUE}┌──({Fore.RED}root㉿kaos-ai{Fore.BLUE})-[{Style.RESET_ALL}~/host_home{Fore.BLUE}]{Style.RESET_ALL}"
    )
    print(line1)
...
def main():
    ...
    while True:
        try:
            ...
            if user_input == "exit":
                print(f"{Fore.RED}🔴 Invalid command. To exit use: kali-exit{Style.RESET_ALL}")
                continue
            
            if user_input == "kali-exit":
                print(f"{Fore.YELLOW}Session ended. Shutting down container...{Style.RESET_ALL}")
                break
            ...
            print(f"🤖 AI Suggests: {Fore.CYAN}{proposed_cmd}{Style.RESET_ALL}")
